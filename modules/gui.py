...
from tkinter.ttk import Frame, Button, Label, Treeview, Scrollbar, Entry, Style
...
def show_blacklist():
    ...
    for key in ['blacklisted_files', 'blacklisted_directories', 'blacklisted_filetypes']:
        if config[key]:
            blacklist_message += f"Blacklisted {key.split('_')[1].capitalize()}: " + ", ".join(config[key]) + "\n"
...
def launch_gui():
    ...
    def on_blacklist():
        ...
        if action in ['add', 'remove']:
            item = simpledialog.askstring("Blacklist Item", "Enter filename, directory, or filetype to blacklist:")
            if item:
    ...
    Button(button_frame, text="Open Directory", command=open_directory).pack(side='left')
    Button(button_frame, text="Organize Files", command=on_organize).pack(side='left')
    Button(button_frame, text="Restore Files", command=on_restore).pack(side='left')
    ...
    Button(button_frame, text="Blacklist", command=on_blacklist).pack(side='left')
    Button(button_frame, text="Show Blacklist", command=on_show_blacklist).pack(side='left')
